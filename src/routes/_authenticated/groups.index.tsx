        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((g, i) => (
            <motion.div key={g.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}>
              <Link to="/groups/$groupId" params={{ groupId: String(g.id) }} className="card-elevated p-5 block hover:border-primary/50 transition group">
                ...
                <div className="mt-4 text-xs text-muted-foreground">{g.player_count} Spieler</div>
              </Link>
            </motion.div>
          ))}
        </div>
...
function CreateGroupDialog({ onCreated }: { onCreated: () => void }) {
  ...
  const m = useMutation({
    mutationFn: async () => {
      await api.groups.create({ name: name.trim(), description: description.trim() || null });
    },
    onSuccess: () => {
