...
import { PENALTY_CATALOG, type PenaltyCatalogItem } from "@/lib/penalty-catalog";
...
function StrafenManager() {
  ...
  const m = useMutation({
    mutationFn: (vars: { targetCode: string; amount: number; crateAmount?: number }) =>
      api.player.penalty({ managerCode: code!, code: vars.targetCode, amount: vars.amount, crateAmount: vars.crateAmount }),
    onSuccess: () => {
  ...
  const rows = (penaltiesQ.data ?? [])
    ...
    .filter((p) => !onlyOutstanding || p.player_penalty > 0 || p.beer_crates > 0)
    .sort((a, b) => b.player_penalty - a.player_penalty || b.beer_crates - a.beer_crates || a.last_name.localeCompare(b.last_name));

  const totalOutstanding = (penaltiesQ.data ?? []).reduce((sum, p) => sum + p.player_penalty, 0);
  const totalCrates = (penaltiesQ.data ?? []).reduce((sum, p) => sum + p.beer_crates, 0);
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 space-y-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            ...
            {" · "}
            <span className="font-semibold text-foreground">{totalCrates}x 🍺</span>
          </p>
        </div>
        ...
        <div className="card-elevated p-5 sm:p-6 space-y-4">
          ...
          {penaltiesQ.isLoading ? (
            ...
            <ul className="divide-y divide-border">
              {rows.map((p) => (
                <PenaltyRow key={p.id} player={p} onApply={(amount, crateAmount) => m.mutate({ targetCode: p.player_code, amount, crateAmount })} pending={m.isPending} />
              ))}
            </ul>
          )}
        </div>
...
function PenaltyRow({
  ...
  onApply: (amount: number, crateAmount?: number) => void;
  pending: boolean;
}) {
  ...
  const apply = (amountValue: number, crateValue: number = 0) => {
    onApply(amountValue, crateValue);
    setOpen(false);
  };
  const applyItem = (item: PenaltyCatalogItem, sign: 1 | -1) => {
    if (item.amount !== undefined) apply(sign * item.amount);
    else if (item.crates !== undefined) apply(0, sign * item.crates);
  };
  ...
  return (
    <li className="flex flex-wrap items-center gap-3 py-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className={`text-xs font-mono ${hasDebt ? "text-destructive" : "text-muted-foreground"}`}>
          {moneyText} | {crateText}
        </div>
      </div>
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Antippen fügt die Strafe direkt hinzu.</DialogDescription>
          </DialogHeader>
                    ...
                    <li key={item.label} className="flex items-center gap-2">
                      ...
                      >
                        ...
                        <span className="font-mono font-semibold shrink-0">
                          {item.amount !== undefined ? `${item.amount.toFixed(2)} €` : `${item.crates}x 🍺`}
                        </span>
                      </button>
                      <button
                        type="button"
                        disabled={pending || (item.amount !== undefined ? player.player_penalty <= 0 : player.beer_crates <= 0)}
                        onClick={() => applyItem(item, -1)}
                        title={item.amount !== undefined ? "Strafe abziehen" : "Kiste abziehen"}
